...
class BirdsSegmentationModel(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss = bce * self.bce_weight + dice * (1 - self.bce_weight) * y.size(0)
        return {'val_loss': loss, 'logs':{'dice':dice, 'bce': bce}, 'iou': iou_pytorch(torch.squeeze(pred, axis=1), torch.squeeze(y, axis=1))}
    ...
    def validation_epoch_end(self, outputs):
        ...
        print(f"[Epoch {self.trainer.current_epoch:3}] Val_loss: {avg_loss:.3f}, Val_dice: {avg_dice:.3f}, Val_bce: {avg_bce:.3f}, Val_IoU: {avg_iou:.3f}", end= " ")
